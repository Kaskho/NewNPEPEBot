...
class BotLogic:
    ...
    def _get_last_run_date(self, task_name):
        ...
        try:
            ...
            logger.error(f"Failed to get last run date from DB for {task_name}: {e}")
            return None
        finally:
            if conn: conn.close()
        ...
        for name, schedule in schedules.items():
            ...
            if should_run:
                try:
                    ...
                    run_marker = (now_utc.strftime('%Y-W%U') if is_weekly else today_utc_str)
    ...
    def _is_spam_or_ad(self, message):
        ...
        if any(keyword in text_lower for keyword in self.FORBIDDEN_KEYWORDS): return True, f"Forbidden Keyword: {keyword}"
    ...
    def send_welcome(self, message):
        ...
        try: self.bot.reply_to(message, welcome_text, reply_markup=self.main_menu_keyboard(), parse_mode="Markdown")
        except Exception as e: logger.error(f"Failed to send welcome: {e}")
        ...
        try:
            ...
            if self.groq_client and self._is_a_question(text):
                ...
                try:
                    ...
                    try: self.bot.edit_message_text(ai_response, chat_id=chat_id, message_id=thinking_message.message_id)
                    except Exception: self.bot.send_message(chat_id, ai_response)
                except Exception as e:
                    ...
                    except Exception as ex: logger.error(f"Failed to send fallback: {ex}")
                return
            ...
            if message.chat.type in ['group', 'supergroup']:
                ...
                if random.random() < current_chance:
                    ...
                    except Exception as e: logger.error(f"Failed to send hype message: {e}")
        except Exception as e:
    ...
    def renew_responses_with_ai(self):
        ...
        if not self.groq_client:
            ...
            return
        try:
            categories_to_renew = { "HYPE": ("Produce 100 short hype messages for a meme coin bot. Funny, enthusiastic, use slang like LFG, WAGMI, ribbit, fren. Each 5-30 words.", 50), "WISDOM": ("Produce 20 wise, motivational quotes for a crypto community. Meme-themed, short, inspiring about HODL, community, moon.", 10) }
            for category, (prompt, min_count) in categories_to_renew.items():
                completion = self.groq_client.chat.completions.create( messages=[{"role": "system", "content": prompt}], model="llama3-8b-8192", temperature=1.0, max_tokens=2000 )
                ...
                if len(new_lines) >= min_count:
                    ...
                    logger.info(f"{category} responses renewed by AI with {len(new_lines)} entries.")
                else:
                    logger.warning(f"AI renewal for {category} returned too few lines ({len(new_lines)}); skipping update.")
        except Exception as e:
